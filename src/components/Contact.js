...
const Contact = ({ data }) => {
  return (
    <section id="contact" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: 'var(--s8) var(--s5)', borderTop: '1px solid var(--rule)' }}>
      <div className="reveal" style={{ width: '100%', maxWidth: '800px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <h2 className="contact-heading" dangerouslySetInnerHTML={{ __html: data.heading }}></h2>
        <p className="contact-sub" style={{ margin: 'var(--s5) auto', maxWidth: '600px', fontSize: '1rem', width: '100%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginTop: 'var(--s5)', width: '100%' }}>
